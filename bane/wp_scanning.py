...
class S0u1wp:
    def __init__(
        ...
        try:
            ...
            self.CheckWordpress = requests.get(
                "http://" + self.url,
            ...
            if self.check == True:
                ...
                print(
                    ...
                    + " URL      : "
                    + m
                    + str(self.url.split(self.wp_path)[0])
                )
                ...
                print(
                    ...
                    + " Server   : "
                    + m
                    + self.CheckWordpress.headers["server"]
                )
                try:
            ...
            print(
                ...
                + "    [ "
                + w
                + " Something worng! target.com without / in end "
                + y
                + "]"
            )
            # sys.exit()
            ...
            print(
                ...
                + "    [ "
                + w
                + " ConnectionError! Maybe server Down, Or your ip is blocked! "
                + y
                + "]"
            )
    ...
    def Worng(self):
        try:
            ...
            print(
                ...
                + "    [ "
                + w
                + " Enter Valid Domain, We Cant Connect to Server "
                + y
                + "]"
            )
        except:
...
|___/|___/     |_|     /_____/ \_____| /_/   |_| |_|  \_| 
    ...
    def UserName_Enumeration(self):
        _cun = 1
        Flag = True
        __Check2 = requests.get(
            "http://" + self.url + "/?author=1",
            timeout=self.timeout,
            proxies=self.proxy,
            headers=self.headers,
            verify=False,
        ...
        try:
            while Flag:
                GG = requests.get(
                    "http://" + self.url + "/wp-json/wp/v2/users/" + str(_cun),
                    timeout=self.timeout,
                    proxies=self.proxy,
                    headers=self.headers,
                    verify=False,
            ...
            try:
                ...
                print(
                    ...
                    + "    [ "
                    + w
                    + " ConnectionError! Maybe server Down, Or your ip blocked! "
                    + y
                    + "]"
                )
    ...
    def CpaNel_UserName_Enumeration(self):
        try:
            ...
            if "/wp-content/" in Get_page.text:
                Hunt_path = requests.get(
                    "http://" + self.url + "/wp-includes/ID3/module.audio.ac3.php",
                    timeout=self.timeout,
                    proxies=self.proxy,
                    headers=self.headers,
                    verify=False,
                ...
                def Hunt_Path_User():
                    try:
                        find = re.findall(
                            "/home/(.*)/public_html/wp-includes/ID3/module.audio.ac3.php",
                            Hunt_path.text,
                        )
                        x = find[0].strip()
                        return x
                ...
                def Hunt_Path_Host():
                    try:
                        find = re.findall(
                            "not found in <b>(.*)wp-includes/ID3/module.audio.ac3.php",
                            Hunt_path.text,
                        )
                        x = find[0].strip()
                        return x
    ...
    def Plugin_NamE_Vuln_TeST(self, Plugin_NaME):
        ...
        while Flag:
            ...
            url = (
                "https://wpvulndb.com/search?page=" + str(num) + "&text=" + Plugin_NaME
            )
            ...
            if "No results" in aa.text or "Cloudflare Ray ID" in aa.text:
                ...
                for x in range(int(bb)):
                    ...
                    self.vulnerabilities.update(
                        {uz: (titles[cal].replace("&lt;", "")).replace("&amp;", "")}
                    )
                    print(
                        r
                        + "["
    ...
    def Version_Wp(self):
        try:
            Check_oNe = requests.get(
                "http://" + self.url + "/readme.html",
                timeout=self.timeout,
                proxies=self.proxy,
                headers=self.headers,
                verify=False,
            ...
            try:
                ...
                print(
                    r + "[" + y + "+" + r + "]" + w + " Wp Version: " + r + "Not Found"
                )
    ...
    def GeT_Theme_Name(self):
        ...
        if "?ver=" in a[0]:
            ...
            if "-" in Name_Theme:
                ...
                print(
                    r + "[" + y + "+" + r + "]" + w + " Themes Name: " + m + Name_Theme
                )
                self.Plugin_NamE_Vuln_TeST(Name_Theme)


def wp_scan(
    u, path="/", check_wp=False, timeout=15, proxy=None, cookie=None, user_agent=None
):
    a = S0u1wp(
        u,
        path=path,
