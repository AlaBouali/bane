...
class Ruby_DAST_Scanner:
    ...
    def scan(u,user_agent=None,cookie=None,timeout=10,logs=True,crt_timeout=120,wayback_timeout=120,subdomain_check_timeout=10,max_wayback_urls=10,subdomains_only=True,headers={},api_key=None,http_proxies=None,socks4_proxies=None,socks5_proxies=None):
        ...
        hed.update(headers)
        try:
            response = requests.Session().get(u+'/rails/info/properties', headers=hed, proxies=Vulnerability_Scanner_Utilities.setup_proxy(proxies), timeout=timeout, verify=False)
            version=Ruby_DAST_Scanner.get_Rails_version(response.text)
        ...
        try:
            ...
            server_os=''
        backend='ruby/{} rubygems/{}'.format(Ruby_DAST_Scanner.get_ruby_version(response.text),Ruby_DAST_Scanner.get_RubyGems_version(response.text))
        if logs==True:
            print("Ruby on Rails site info:\n\n\tURL: {}\n\tDomain: {}\n\tIP: {}\n\tServer: {}\n\tOS: {}\n\tBackend technology: {}\n\tRails version: {}\n".format(u,domain,ip,server,server_os,backend,version))
        ...
        if version!='':
            ...
            wpvulns=Vulners_Search_Scanner.scan('ruby on rails',version=version,proxy=Vulnerability_Scanner_Utilities.setup_proxy(proxies),api_key=api_key)
            for x in wpvulns:
                if 'rails' in x['title'].lower() or 'rails' in x['description'].lower():
                    wp_vulns.append(x)
            for x in wp_vulns:
            ...
            for sv in server.split():
                if sv.startswith('(')==False:
                    ...
                    if logs==True:
                        print('[i] looking for exploits for : {}\n'.format(sv))
                    if '/' in sv:
                        sv_e=Vulners_Search_Scanner.scan(sv.split('/')[0].replace('Microsoft-IIS','microsoft internet information services').lower(),version=sv.split('/')[1],proxy=Vulnerability_Scanner_Utilities.setup_proxy(proxies),api_key=api_key)
                    else:
                        if logs==True:
                            print('\t[-] unknown version\n')
                    for x in sv_e:
