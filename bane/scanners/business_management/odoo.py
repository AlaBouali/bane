...
class Odoo_Scanner:
    ...
    @staticmethod
    def scan(u,version='',user_agent=None,cookie=None,timeout=10,logs=True,crt_timeout=120,wayback_timeout=120,subdomain_check_timeout=10,max_wayback_urls=10,subdomains_only=True,headers={},api_key=None,http_proxies=None,socks4_proxies=None,socks5_proxies=None):
        ...
        hed.update(headers)
        try:
            response = requests.Session().post(u+'/xmlrpc/2/common', headers=hed, proxies=Vulnerability_Scanner_Utilities.setup_proxy(proxies), timeout=timeout, verify=False)
        ...
        try:
            ...
            server_os=''
        try:
            backend='python/'+response.text.lower().split('/python')[1].split('/')[0].strip()
        except:
            backend='python'
        if logs==True:
            print("Odoo site info:\n\n\tURL: {}\n\tDomain: {}\n\tIP: {}\n\tServer: {}\n\tOS: {}\n\tBackend technology: {}\n\tOdoo version: {}\n".format(u,domain,ip,server,server_os,backend,version))
        ...
        if version!='':
            ...
            wpvulns=Vulners_Search_Scanner.scan('odoo',version=version,proxy=Vulnerability_Scanner_Utilities.setup_proxy(proxies),api_key=api_key)
            for x in wpvulns:
                if 'odoo' in x['title'].lower() or 'odoo' in x['description'].lower():
                    wp_vulns.append(x)
            for x in wp_vulns:
