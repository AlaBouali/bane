...
class PHP_DAST_Scanner:
    ...
    def scan(u,user_agent=None,cookie=None,timeout=10,logs=True,crt_timeout=120,wayback_timeout=120,subdomain_check_timeout=10,max_wayback_urls=10,subdomains_only=True,headers={},api_key=None,http_proxies=None,socks4_proxies=None,socks5_proxies=None):
        ...
        if version!='':
            ...
            wpvulns=Vulners_Search_Scanner.scan('php',version=version,proxy=Vulnerability_Scanner_Utilities.setup_proxy(proxies),api_key=api_key)
            for x in wpvulns:
                if 'php' in x['title'].lower() or 'php' in x['description'].lower():
                    wp_vulns.append(x)
            for x in wp_vulns:
